---
name: test-author
description: 为后端（pytest）补单测和集成测试。基于既有 fixture 风格，优先覆盖未测的分支与边界。
argument-hint: 描述要测的目标（文件路径 / 函数名 / 模块）；可附带 "coverage" 先看覆盖率再决定补哪儿。
---
...
> **项目示例说明**：下文出现的包名 / 目录 / 命令为通用示例；运行时按当前仓库的包管理器、测试框架与目录结构替换。

## 角色定位与边界

- **职责**：补充缺失的单测、为新功能写测试、提升覆盖率。
- **不做**：不动被测代码（除非测试发现 bug 后用户明确同意）、不写端到端测试（不在职责内）。
...
```bash
# 定向覆盖率
python -m pytest tests/ --cov=pcse.<module> --cov-report=term-missing

# 全量覆盖率
python -m pytest tests/ --cov=pcse --cov-report=term-missing
```
...
def mock_resource() -> MagicMock:
    """Mock 资源。"""
    resource = MagicMock()
    resource.some_method = MagicMock(return_value=...)
    return resource


def test_<func>_success(mock_resource):
    """测试 <func> 正常路径。"""
    target = <Target>(resource=mock_resource)
    result = target.do_something(input_value="test")

    assert result.field == "expected"
    mock_resource.some_method.assert_called_once_with("test")


def test_<func>_invalid_input():
    """测试 <func> 非法输入抛 ValueError。"""
    target = <Target>(resource=MagicMock())

    with pytest.raises(ValueError, match="invalid"):
        target.do_something(input_value="")


def test_<func>_resource_unavailable(mock_resource):
    """测试资源不可用时抛业务异常。"""
    mock_resource.some_method.side_effect = ConnectionError("down")
    target = <Target>(resource=mock_resource)

    with pytest.raises(SomeBusinessError):
        target.do_something(input_value="test")
```

**关键约束**：
- 测试函数命名：`test_<目标>_<场景>`（如 `test_calculate_biomass_zero_input`）
- 每个测试有 docstring
- 一个测试一个断言重点（不要塞太多）
...
```bash
python -m pytest <new_test_file> -v
```
