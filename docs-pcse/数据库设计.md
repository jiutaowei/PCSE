...
**注意**：Demo 数据库仅支持 **单层土壤**（`fetch_soildata()` 强制检查 `len(rows) == 1`）。多层数据存在但使用 `fetch_soiltype_multilayer` 另有函数处理。
...
```
用户调用 engine.start_wofost(grid=31031, crop=1, year=2000, mode='wlp')
    │
    ├─ 1. 连接 SQLite 数据库 (~/.cropsim/cropsim.db)
    │
...
## 10. Demo 数据库 vs CGMS 生产数据库

| 特性 | Demo DB | CGMS 生产 DB |
|------|---------|-------------|
| 数据库文件 | `~/.cropsim/cropsim.db` | 中心化服务器 |
| 数据规模 | 3 个网格, 17 种作物, 2 年 | 全欧 10000+ 网格, 数十年 |
| 天气数据 | 完整每日气象要素 | 相同结构 |
| 集合预报 | 50 成员 × 1 年 | 多年集合 |
| 任务管理 | tasklist 表 (手工) | TaskManager (SQLAlchemy) |
| 土壤 | 仅单层土壤 | 支持多层 |
| 构建方式 | `cropsim_dump.sql` 自动加载 | 外部 ETL 管道 |
